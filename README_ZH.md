...
### 3. 物理整形逻辑说明
由于计时器输出的是微弱的模拟音频信号（正弦波），带有噪声。
1. **模拟阶段**：计时器信号（约 0.7V~2.5V 波动）进入 LM393 的 `IN+`。
2. **比较阶段**：LM393 将其与 `IN-`（固定在 1.65V）进行实时比较。
3. **数字阶段**：信号 > 1.65V -> 输出 3.3V；信号 < 1.65V -> 输出 0V。
4. **结果**：输出端产生标准的数字方波，由 ESP32 的硬件串口 (UART) 精准解析 ASCII 字符。

### 4. 硬件资源
<img src="./hardware/Case/caes-render.jpg" width="400" alt="外壳渲染图"> <img src="./hardware/PCB/PCB.png" width="350" alt="PCB 预览图">

- **PCB 设计** (`hardware/PCB/`)：包含打样所需的 Gerber 文件与工程源文件。
- **3D 打印外壳** (`hardware/Case/`)：包含导出好的 STL 模型及 Blender 源文件。
...
4. 打开 `stackmatlink.ino`，选择您的 ESP32-S3 开发板型号并点击 **上传**。
   - *注：如果在 IDE 中启用了 USB CDC On Boot，请使用 Native USB 端口进行调试。*

### 2. 使用指南
1. **上电**：将 ESP32-S3 接入电源，并将音频线连接到计时器。
2. **确认信号**：打开串口监视器（115200 波特率）。计时开始时，指示灯将变为**蓝色**。
3. **连接 csTimer**：
    - 打开 [csTimer.net](https://cstimer.net) (推荐使用 Chrome/Edge)。
    - 设置 -> 计时器 -> 输入类型 -> **蓝牙魔方/计时器**。
    - 点击 csTimer 顶部蓝牙图标，选择 **"GAN-Timer"** 并配对。
4. **开始计时**：您的物理计时器操作将实时同步到网页中。

## ⚠️ 注意事项 / 故障排除

- **信号解析失败**：如果串口显示 GPIO 有电平变化但无法解析，请尝试在代码 `stackmatTask` 中将 `inverted` 初始值取反。
- **设备连接**：Chrome (Windows/macOS/Android) 或 Bluefy (iOS) 是目前 Web Bluetooth 兼容性最好的方案。
- **指示灯状态**：**蓝色**（计时中），**白色/闪烁**（就绪/重置）。
...
### 鸣谢
- `NimBLE-Arduino` 与 `Adafruit_NeoPixel` 库开发者。
